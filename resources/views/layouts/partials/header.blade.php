            ...
            <div class="flex items-center space-x-4 flex-shrink-0">
                ...
                <div class="relative" x-data="{ open: false }" @click.away="open = false">
                    <button @click="open = !open"
                            class="flex items-center space-x-2 px-3 py-2.5 bg-gray-800/70 border border-gray-600/50 rounded-xl text-gray-300 hover:text-white hover:border-yellow-500/50 transition-all duration-300 group">
                    ...
                    <div x-show="open"
                         ...
                         class="absolute right-0 mt-3 w-40 bg-gray-800/95 backdrop-blur-lg border border-gray-700/50 rounded-2xl shadow-2xl shadow-black/50 overflow-hidden">
                        <div class="py-2">
                            <a href="{{ route('language.switch', 'ru') }}"
                               class="flex items-center space-x-3 px-4 py-2.5 text-gray-300 hover:text-white hover:bg-yellow-500/10 transition-colors {{ app()->getLocale() == 'ru' ? 'bg-yellow-500/10 text-yellow-400' : '' }}">
                                <span class="text-lg">🇷🇺</span>
                                <span class="text-sm font-medium">Русский</span>
                                @if(app()->getLocale() == 'ru')
                                    <i class="fas fa-check text-yellow-400 ml-auto"></i>
                                @endif
                            </a>
                            <a href="{{ route('language.switch', 'en') }}"
                               class="flex items-center space-x-3 px-4 py-2.5 text-gray-300 hover:text-white hover:bg-yellow-500/10 transition-colors {{ app()->getLocale() == 'en' ? 'bg-yellow-500/10 text-yellow-400' : '' }}">
                                <span class="text-lg">🇬🇧</span>
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                ...
                                @empty
                                    <div class="col-span-2 text-center text-gray-400 py-4">
                                        ...
                                        <p class="text-sm">{{ __('header.no_genres') }}</p>
                                    </div>
                                @endforelse
                            </div>
                        </div>
                    ...
                    <div class="relative" x-data="{ open: false }" @click.away="open = false">
                        <button @click="open = !open"
                            ...
                            <div class="w-8 h-8 rounded-full bg-gradient-to-r from-yellow-400 to-orange-500 flex items-center justify-center text-gray-900 font-bold shadow-lg shadow-yellow-500/30">
                                <span class="text-sm">{{ strtoupper(substr(Auth::user()->name ?? 'A', 0, 1)) }}</span>
                            </div>
                            <span class="text-sm text-gray-300 group-hover:text-white transition-colors">
                                {{ Auth::user()->name ?? __('header.user') }}
                            </span>
                            ...
                            <div class="py-2">
                                ...
                                <a href="#"
                                   class="flex items-center space-x-3 px-4 py-2.5 text-gray-300 hover:text-white hover:bg-yellow-500/10 transition-colors">
                                    ...
                                    <span>{{ __('header.watch_history') }}</span>
                                </a>
                                <a href="#"
                                   class="flex items-center space-x-3 px-4 py-2.5 text-gray-300 hover:text-white hover:bg-yellow-500/10 transition-colors">
